...
array_shift($args);
$jobName = array_pop($args);
if (null == $jobName || $jobName[0] == '-') {
    help();
}
...
$jobManager->addFeature(new Job\Feature\OutputConsole());
$job = $jobManager->getJob($jobName);
if (null === $job) {
    echo "Job '$jobName' not found" . PHP_EOL;
    exit(1);
}
$job->run();
$job->save();
